...
namespace Bethel_Heritage_Portal_Web.Pages
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostTermResult(int session, string term, int Classess)
        {
            if (session > 0 && !string.IsNullOrEmpty(term))
            {
                var ClaimsId = (ClaimsIdentity)User.Identity;
                var claim = ClaimsId.FindFirst(ClaimTypes.NameIdentifier);
                var result = dbContext.TermRegistrations.Include(j => j.StudentsData.ApplicationUser).Include(j => j.SessionYear).Include(j => j.Schoolclasses).Include(j => j.SubClasses).FirstOrDefault(k => k.StudentsData.ApplicationUserId == claim.Value && k.Term == term && k.SessionYearId == session && k.ClassesInSchoolId == Classess);
                if (result != null)
                {
